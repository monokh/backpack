...
const generatePropRows = propInfo => {
  ...
  for (let i = 0; i < Object.keys(propInfo).length; i += 1) {
    ...
    const listToPush = newPropRow.required
      ? requiredPropRows
      : optionalPropRows;
...
const prettyFormatPropRows = (
  requiredPropRowsSorted,
  optionalPropRowsSorted,
) => {
  const header = [
    {
      name: 'Property',
  ...
  const tableData = header
    .concat(requiredPropRowsSorted)
    .concat(optionalPropRowsSorted);
  const { columnCount, rowCount, columnWidths } = calculateColumnWidths(
    tableData,
  );
  let tableString = '';
...
const updateProps = (componentSourceFile, componentReadmeFilePath) => {
  try {
    const componentInfo = execSync(`react-docgen ${componentSourceFile}`)
      .toString()
      .split('\n');
    const propInfo = JSON.parse(componentInfo[0]).props;
    ...
    const tableString = prettyFormatPropRows(
      requiredPropRowsSorted,
      optionalPropRowsSorted,
    );
